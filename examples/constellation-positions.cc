...
void
logLocation (const Ptr<Constellation> &constellation)
{
      ...
      for (auto index = 0; index < constellation->GetPlaneSize (); index++)
        {
          ...
          NS_LOG_DEBUG ("Satellite: ("
                        << plane << ", " << index << ") is at: " << mobility->GetPosition ()
                        << " i.e., "
                        << CartesianToGeographicCoordinates (mobility->GetPosition (),
                                                             GeographicPositions::WGS84));
        }
    }

  ns3::Simulator::Schedule (Seconds (1.0), logLocation, constellation);
}
} // namespace
